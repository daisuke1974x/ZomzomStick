...
//#include <M5StickC.h>
//int M5StickSystem = 0;// 0:M5Stick  1:M5Stick Plus
#include <M5StickCPlus.h>
int M5StickSystem = 1;// 0:M5Stick  1:M5Stick Plus
...
class MyCallbacks : public BLEServerCallbacks {
  ...
  void onDisconnect(BLEServer* pServer){
    ...
    if (M5StickSystem == 1){
      M5.Lcd.println(" [ Zomzom Stick ]");
      M5.Lcd.println(" Status:Disonnected");
    }
    flag_pairing = false;
  }
};
  ...
  if (M5.BtnA.pressedFor(500)) {
    if(connected){
      if(flag_BtnA == false){
        ...
        if (flag_os == 1){
          // macのとき Command(⌘)+Shift+Vでビデオを開始/停止する。 Commandキーのコードは、WINキーと同じらしい。
          uint8_t keys[] = { 0x19 };//V
    ...
    if( M5.BtnA.wasReleased() ){
      if(connected){
        if(flag_BtnA == false){//長押し操作のときは除く
          ...
          if (flag_os == 0){
            // Windowsのとき Alt+Aでオーディオをミュート/ミュート解除する。
            uint8_t keys[] = { 0x04 };//A
            sendKeys(KEY_MASK_ALT, keys, 1);
          }
          if (flag_os == 1){
            // macのとき Command(⌘)+Shift+A：オーディオをミュート/ミュート解除する。 Commandキーのコードは、WINキーと同じらしい。
            uint8_t keys[] = { 0x04 };//A
            sendKeys(KEY_MASK_WIN +KEY_MASK_SHIFT, keys, 1);
          }
